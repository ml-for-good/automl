...
def create_detection_service():
  ...
  def train():
    ...
    if train_dataset.endswith('csv') and test_dataset.endswith('csv'):
      ...
      train_data, validation_data, _ = object_detector.DataLoader.from_csv(train_dataset)
      _, _, test_data = object_detector.DataLoader.from_csv(test_dataset)
      task.status = 'training'
      task.model = object_detector.create(train_data, model_spec=spec, batch_size=batch_size, validation_data=validation_data, epochs=epochs)
  ...
  def view():
    ...
    return {'msg': 'Current tasks' + str(len(manager.tasks)), 'tasks':tasks}
  ...
  def export():
    ...
    if task is None:
      return {'msg': 'Task is not found'}
    task.model.export(export_dir=url, export_format=[ExportFormat.SAVED_MODEL, ExportFormat.LABEL])
...
def create_nlp_service():
  ...
  def train():
    ...
    if train_dataset.endswith('csv') and test_dataset.endswith('csv'):
      task.status='training'
      train_data = text_classifier.DataLoader.from_csv(train_dataset, is_training=True,
                                                       text_column='sentence',
                                                       label_column='label',
                                                       model_spec=spec)
      test_data = text_classifier.DataLoader.from_csv(test_dataset, is_training=False,
                                                      text_column='sentence',
                                                      label_column='label',
                                                      model_spec=spec)
      task.model = text_classifier.create(train_data, model_spec=spec, batch_size=batch_size, epochs=epochs)
      task.status = 'evaluating'
      metric = task.model.evaluate(test_data)
      return {'msg': 'Train finished', 'metric': metric}
    else:
